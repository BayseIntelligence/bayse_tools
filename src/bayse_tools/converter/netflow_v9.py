"""
    ...
    This file contains the Netflow class, which captures the fields that are found in v9 of Cisco's Netflow v9 format
    for network traffic. Note that there are MANY fields in Netflow that will not be a part of this converter, as they
    are not necessary to convert to BayseFlow. The file is expected to contain one (REPLACE Netflow per line in JSON
    ...
    def __init__(self, flowdata, is_json=False):
        ...
        if is_json:
            try:
                ...
                srcip_vals = netflowutils.json_extract(flowdata, "srcip")
                srcport_vals = netflowutils.json_extract(flowdata, "srcport")
                dstip_vals = netflowutils.json_extract(flowdata, "dstip")
                dstport_vals = netflowutils.json_extract(flowdata, "dstport")
                ts_vals = netflowutils.json_extract(flowdata, "timestamp")

                # fields that we can possibly interpret if only some are missing
                duration_vals = netflowutils.json_extract(flowdata, "duration")
                outbytes_vals = netflowutils.json_extract(flowdata, "outbytes_total")
                inbytes_vals = netflowutils.json_extract(flowdata, "inbytes_total")
                totalbytes_vals = netflowutils.json_extract(flowdata, "totalbytes")
                outpkts_vals = netflowutils.json_extract(flowdata, "outpkts_delta")
                total_outpkts_vals = netflowutils.json_extract(flowdata, "outpkts_total")
                inpkts_vals = netflowutils.json_extract(flowdata, "inpkts_delta")
                ...
                if len(missing) > 0:  # short-circuit here first if failed with absolutely mandatory fields
                    print(f"Mandatory fields missing from JSON record for Netflow: {missing}")
                    self.bayseflow_key = None
                    return
                ...
                dstip_host_vals = netflowutils.json_extract(flowdata, "dstip_host")
                proto_vals = netflowutils.json_extract(flowdata, "proto_name")
                service_vals = netflowutils.json_extract(flowdata, "service")
