"""
    ...
    This file contains the ipfix class, which captures the fields that are found in v9 of Cisco's ipfix v9 format
    for network traffic. Note that there are MANY fields in ipfix that will not be a part of this converter, as they
    are not necessary to convert to BayseFlow. The file is expected to contain one (REPLACE ipfix per line in JSON
    format, and any DNS Interflow records (if provided) are expected to also be contained within the file. For
    non-DNS records,
...
from bayse_tools.converter import ipfixutils
import math
    ...
    def __init__(self, flowdata, is_json=False):
        ...
        if is_json:
            try:
                ...
                srcip_vals = ipfixutils.json_extract(flowdata, "srcip")
                srcport_vals = ipfixutils.json_extract(flowdata, "srcport")
                dstip_vals = ipfixutils.json_extract(flowdata, "dstip")
                dstport_vals = ipfixutils.json_extract(flowdata, "dstport")
                ts_vals = ipfixutils.json_extract(flowdata, "timestamp")

                # fields that we can possibly interpret if only some are missing
                duration_vals = ipfixutils.json_extract(flowdata, "duration")
                outbytes_vals = ipfixutils.json_extract(flowdata, "outbytes_total")
                inbytes_vals = ipfixutils.json_extract(flowdata, "inbytes_total")
                totalbytes_vals = ipfixutils.json_extract(flowdata, "totalbytes")
                outpkts_vals = ipfixutils.json_extract(flowdata, "outpkts_delta")
                total_outpkts_vals = ipfixutils.json_extract(flowdata, "outpkts_total")
                inpkts_vals = ipfixutils.json_extract(flowdata, "inpkts_delta")
                total_inpkts_vals = ipfixutils.json_extract(flowdata, "inpkts_total")
                totalpkts_vals = ipfixutils.json_extract(flowdata, "totalpackets")
                ...
                if len(missing) > 0:  # short-circuit here first if failed with absolutely mandatory fields
                    print(f"Mandatory fields missing from JSON record for ipfix: {missing}")
                ...
                dstip_host_vals = ipfixutils.json_extract(flowdata, "dstip_host")
                proto_vals = ipfixutils.json_extract(flowdata, "proto_name")
                service_vals = ipfixutils.json_extract(flowdata, "service")
                ...
                print("Something went wrong while trying to parse JSON record for ipfix:\n{}".format(e))
                self.bayseflow_key = None
        else:
            print(f"ipfix data not recognized. We only accept ipfix records in JSON format, with each record "
